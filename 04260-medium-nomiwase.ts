...
// ============= Your Code Here =============
// 字符串转联合类型
type S2U<T extends string> =   T extends `${infer L}${infer R}` ?  L | S2U<R> : T
// 
type AllCombinations<A extends string,C extends string = S2U<A>> = [C] extends [never] ? '' : 
  '' | { [K in C]: `${K}${AllCombinations<never, Exclude<C, K>>}`}[C]
